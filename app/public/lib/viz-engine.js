 ...
 * clears the viz DOM, and resets the booted flag so boot(D) can be called again
 * with a fresh bundle (used by the Rebuild button in the Mac app). */
window.__MONTAUK_VIZ__.reset = function resetMontaukViz() {
  const state = window.__MONTAUK_VIZ__._state;
  if (state) {
    for (const dispose of state.disposers) {
      try { dispose(); } catch (e) { /* noop — listener already gone */ }
    }
    for (const chart of state.charts) {
      try { chart.remove(); } catch (e) { /* noop — chart already removed */ }
    }
  }

  const innerHostIds = [
    "chart-price", "chart-equity", "chart-health",
    "strat-list", "r-meta", "r-metrics", "r-era-breakdown",
    "r-market-regimes", "r-gates", "r-params", "r-health",
    "r-scorecards", "r-val-summary",
  ];
  for (const id of innerHostIds) {
    const el = document.getElementById(id);
    if (el) el.innerHTML = "";
  }
  for (const id of ["active-name", "hover-readout", "r-name", "r-codename", "prov-text", "generated-badge"]) {
    const el = document.getElementById(id);
    if (el) el.textContent = "";
  }
  for (const id of ["r-era-header", "r-regime-header"]) {
    const el = document.getElementById(id);
  ...
  window.__MONTAUK_VIZ__.refit = null;
};

window.__MONTAUK_VIZ__.boot = function bootMontaukViz(D) {
  "use strict";

  if (window.__MONTAUK_VIZ__.booted) return;
  window.__MONTAUK_VIZ__.booted = true;

  if (!D) {
    const host = document.getElementById("viz-shell") || document.body;
    host.innerHTML = '<div style="color:#f06666;padding:30px;font-family:monospace">' +
      'No bundle payload received. Run build_viz.py or check the read_viz_bundle bridge.</div>';
    return;
  }
  ...
  /* ---- Chart factory shared options (synthwave palette) ---- */
  const baseChartOpts = {
    layout: {
      background: { type: "solid", color: "#080a0d" },
      textColor: "#7f908d",
      fontSize: 11,
      fontFamily: 'Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",Helvetica,Arial,sans-serif',
    },
    grid: {
      vertLines: { color: "#172028" },
      horzLines: { color: "#172028" },
    },
    rightPriceScale: {
      borderColor: "#24303a",
    },
    ...
    crosshair: {
      ...
      vertLine: { color: "rgba(255, 95, 210, 0.42)", width: 1, style: 3, labelBackgroundColor: "#172027" },
      horzLine: { color: "rgba(99, 199, 214, 0.38)", width: 1, style: 3, labelBackgroundColor: "#172027" },
    },
  ...
  _charts.push(priceChart, equityChart, healthChart);
  ...
  _on(indexedToggle, "change", () => applyEquityDrawdownSeries());
  ...
  const candleSeries = priceChart.addLineSeries({
    ...
    crosshairMarkerBorderColor: "#63c7d6",
  ...
  if (tecl.synthetic_end_index >= 0 && tecl.synthetic_end_index < tecl.dates.length) {
    ...
    const tintSeries = priceChart.addAreaSeries({
      topColor: "rgba(124, 92, 255, 0.10)",
      bottomColor: "rgba(124, 92, 255, 0.02)",
      lineColor: "rgba(124, 92, 255, 0)",
      lineWidth: 1,
  ...
  function renderSeamBoundary() {
    ...
    line.style.top = "0";
    line.style.bottom = "0";
    line.style.borderLeft = "1px dashed rgba(124, 92, 255, 0.75)";
    line.style.boxShadow = "0 0 12px rgba(124, 92, 255, 0.32)";
    ...
    label.style.border = "1px solid rgba(124, 92, 255, 0.48)";
    label.style.borderRadius = "3px";
    label.style.background = "rgba(8, 10, 13, 0.9)";
    label.style.color = "#7c5cff";
    label.style.fontSize = "11px";
    label.style.letterSpacing = "0.3px";
    label.style.whiteSpace = "nowrap";
    label.style.boxShadow = "0 0 14px rgba(124, 92, 255, 0.22)";
  ...
  const healthBandSeries = healthChart.addBaselineSeries({
    ...
    topLineColor: "rgba(66, 214, 154, 0.92)",
    bottomLineColor: "rgba(240, 102, 102, 0.92)",
    topFillColor1: "rgba(66, 214, 154, 0.18)",
    topFillColor2: "rgba(66, 214, 154, 0.02)",
    bottomFillColor1: "rgba(240, 102, 102, 0.02)",
    bottomFillColor2: "rgba(240, 102, 102, 0.16)",
    lineWidth: 2,
    priceLineVisible: false,
    lastValueVisible: true,
    title: "Health",
    ...
    if (includeNorthStar && D.markers && D.markers.north_star) {
      D.markers.north_star.forEach((ev) => {
          m.push({
            ...
            color: ev.type === "buy" ? "#7c5cff" : "#7c5cff",
          shape: "circle",
  ...
  // Expose a refit hook so the app can ask the charts to resize after the
  // Viz tab regains layout width (display:none → display:block transitions).
  window.__MONTAUK_VIZ__.refit = fitAll;

  // Lifecycle state for __MONTAUK_VIZ__.reset() — see the reset() function
  // above. Holds chart instances + listener-disposer closures.
  window.__MONTAUK_VIZ__._state = { charts: _charts, disposers: _disposers };
};
